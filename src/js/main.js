 ...
 * @Last Modified time: 2019-05-29 14:18:40
 */
(function() {
	...
	// 轮播图背景颜色
	var bg_color = ['rgb(254, 187, 184)', 'rgb(254, 246, 249)', 'rgb(255, 255, 255)', 'rgb(255, 255, 255)', 'rgb(211, 231, 248)', 'rgb(235, 212, 246)', 'rgb(255, 255, 255)'];
	//轮播图
...
//  	noSwipingSelector: 'span',
	...
	//点击商品跳转详情页
	//循环绑定事件
	$('#oneF .oneF_content ul li').each(function(i, item) {
		$('#oneF .oneF_content ul li').eq(i).attr('data-gid', i + 1);
		// console.log($(this).eq(i).('id'))
	...
	// window.open('detail.html?'+$(this).eq);

	//          for(var i = 0; i < btns.length; i++) {
	//              //绑定索引
	//              btns[i].index = i;
	//              btns[i].onclick = function() {
	// //               console.log('点击了'+ this.index);//点哪个就获取哪个的索引值
	//                  console.log(data[this.index]);//点击哪条数据就用哪条数据
	//                  var str = objToStr(data[this.index]);
	//                  console.log(str);
	//                  location.href = '14详情页.html?' + str;
	//              }
	//          }
	//      //页面加载刷新购物车渲染
	// var carnum = 0;
